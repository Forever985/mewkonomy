...
if ((ldSearchData.value as any).priceType != null && ldSearchData.value.productPriceType == null) {
  ldSearchData.value.productPriceType = (ldSearchData.value as any).priceType
}
delete (ldSearchData.value as any).priceType
delete (ldSearchData.value as any).minLevel
delete (ldSearchData.value as any).maxLevel

/** 材料买价 / 成品售价的可选口径（须在 panelFields 之前声明，否则配置求值时处于暂时性死区） */
...
const panelFields: PanelField[] = [
  { type: "sort", key: "sortRules", label: "排序优先级", fields: ENHANCEEXP_SORT_FIELDS, defaultProp: "result.costPerExp" },
  { type: "name", key: "name", label: "物品", width: 260 },
  {
    type: "conditions",
    label: "只看目标等级",
    stepsCount: 20,
    ...
    hint: "多选后仅显示这些强化等级的方案"
  },
  { type: "checkbox", key: "onlyProfitable", label: "仅看赚钱方案" },
  { type: "range", label: "最少经验", maxKey: "minExp", min: 0, width: 110, placeholderMax: "0" },
