...
export async function getPriceDataApi(params: RequestData) {
  ...
  // 多物品名称筛选：支持 string 或 string[]
  const names = Array.isArray(params.name) ? params.name.filter(Boolean) : params.name ? [params.name] : []
  names.length && (list = list.filter(item => names.some(n => getTrans(getItemDetailOf(item.hrid).name).toLocaleLowerCase().includes(String(n).toLowerCase()))))
