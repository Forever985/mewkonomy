export default {
  ...
  "市场监控口径说明": "本頁資料來自官方市場快照：成交量是「當日累計成交量」（每天 UTC 0 點歸零），並不等於即時熱度；跨過 UTC 零點後所有累計量會重新從小數字開始。要看「最近多熱」請用「成交量/小時」欄 —— 它比較兩個快照之間的增量。快照本身約每小時才更新一次，所以時間窗越小，實際能用的估算區間往往越長（見欄頭說明）。",
  "成交量口径说明": "當日累計成交量（官方 v 欄位），每天 UTC 0 點歸零。因此它反映的是「從今天 0 點到現在一共成交了多少」，而不是目前速度；剛過 UTC 零點時所有數字都會很小，屬正常現象。",
  ...
  "成交额口径说明": "按「當日累計成交量 × 目前價」估算，不是真實成交額：當天各筆成交的價格並不相同，這裡統一用目前價折算；且累計量每天 UTC 0 點歸零，所以它同樣是「今日至今」的口徑。",
  ...
  "监控各物品市场成交量与成交额，按当日累计成交量排行": "監控各物品市場成交量與成交額；成交量是當日累計值（UTC 0 點歸零），非即時熱度",
}