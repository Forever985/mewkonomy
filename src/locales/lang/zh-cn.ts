export default {
  ...
  "市场监控口径说明": "本页数据来自官方市场快照：成交量是「当日累计成交量」（每天 UTC 0 点归零），并不等于实时热度；跨过 UTC 零点后所有累计量会重新从小数字开始。要看\"最近多热\"请用\n「成交量/小时」列——它比较两个快照之间的增量。快照本身约每小时才更新一次，所以时间窗越小，实际能用来估算的区间往往越长（见列头说明）。",
  "成交量口径说明": "当日累计成交量（官方 v 字段），每天 UTC 0 点归零。因此它反映的是\"从今天 0 点到现在一共成交了多少\"，而不是当前速度；刚过 UTC 零点时所有数字都会很小，属正常现象。",
  ...
  "成交额口径说明": "按「当日累计成交量 × 当前价」估算，不是真实成交额：当天各笔成交的价格并不相同，这里统一用当前价折算；且累计量每天 UTC 0 点归零，所以它同样是\"今日至今\"的口径。",
  ...
  "监控各物品市场成交量与成交额，按当日累计成交量排行": "监控各物品市场成交量与成交额；成交量是当日累计值（UTC 0 点归零），非实时热度",
}